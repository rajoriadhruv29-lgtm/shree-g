...
const Navbar: React.FC = () => {
  ...
  const navLinkClass = "text-brand-charcoal hover:text-brand-blue font-medium transition-colors px-3 py-2";
        ...
        <div className="flex justify-between h-24">
          ...
          <div className="hidden md:flex items-center space-x-4">
            ...
            >
              ...
              <div className={`absolute left-0 mt-0 w-56 bg-white rounded-md shadow-lg ring-1 ring-black ring-opacity-5 transition-all duration-200 ${dropdownOpen ? 'opacity-100 visible translate-y-0' : 'opacity-0 invisible -translate-y-2'}`}>
                <div className="py-1">
                  <Link to="/products" className="block px-4 py-2 text-sm text-brand-charcoal hover:bg-brand-gray hover:text-brand-blue border-b border-gray-100">{t('nav.allProducts')}</Link>
                  {CATEGORIES.map((cat) => (
                    <Link 
                      key={cat.id}
                      to={`/products/${cat.id}`} 
                      className="block px-4 py-2 text-sm text-brand-charcoal hover:bg-brand-gray hover:text-brand-blue"
                    >
            ...
            <Link to="/contact" className="bg-brand-blue text-white px-5 py-2 rounded-full font-medium hover:bg-brand-charcoal transition-all shadow-md border border-transparent hover:border-brand-yellow">{t('nav.contact')}</Link>
          ...
          <div className="flex items-center md:hidden gap-4">
            ...
            <button
              ...
              className="inline-flex items-center justify-center p-2 rounded-md text-brand-charcoal hover:text-brand-blue focus:outline-none"
            >
      ...
      {isOpen && (
        <div className="md:hidden bg-white border-t border-gray-200">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            <Link to="/" className="block px-3 py-2 rounded-md text-base font-medium text-brand-charcoal hover:text-brand-blue hover:bg-gray-50">{t('nav.home')}</Link>
            <Link to="/about" className="block px-3 py-2 rounded-md text-base font-medium text-brand-charcoal hover:text-brand-blue hover:bg-gray-50">{t('nav.about')}</Link>
            ...
            <Link to="/blog" className="block px-3 py-2 rounded-md text-base font-medium text-brand-charcoal hover:text-brand-blue hover:bg-gray-50">{t('nav.blog')}</Link>
