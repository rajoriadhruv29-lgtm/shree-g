...
const ProductDetail: React.FC = () => {
  ...
  const jsonLd = [
    ...
    breadcrumbJsonLd(SITE_URL, [
      ...
      { name: category ? category.title : product.categoryId, path: `/products/${product.categoryId}` },
      { name: product.name, path: `/products/${product.categoryId}/${product.id}` },
    ]),
  ];
  ...
  return (
    <div className="min-h-screen bg-white py-12">
      <Seo title={seo.title} description={seo.description} path={`/products/${product.categoryId}/${product.id}`} image={product.imageUrl} jsonLd={jsonLd} />
